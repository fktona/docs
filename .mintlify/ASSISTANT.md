...
- The TypeScript SDK (`@vela/sdk`) and Python SDK (`vela-sdk`) are the current client libraries.
- The CLI (`@vela/cli`) provides a Prisma-like schema-as-code workflow — define schemas locally as JSON files and sync with `vela push`, `vela pull`, and `vela diff`.
